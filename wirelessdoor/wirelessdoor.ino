...
String html_2 = "<form id='F1' action='LOCKON'><input class='button' type='submit' value='Open The Door' ></form><br>";
...
void loop() 
{
    ...
    if       ( request.indexOf("LOCKON") > 0 )  { digitalWrite(LED_Pin, LOW);  }
    else if  ( request.indexOf("LOCKOFF") > 0 ) { digitalWrite(LED_Pin, HIGH);   }
    client.flush();
    client.print( header );
    client.print( html_1 );
    client.print( html_2 );
